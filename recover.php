    ...
    if(isset($_SESSION['token']) && $email_check === 'ok' && $_POST['token'] == $_SESSION['token']){
      ...
      $count = mysqli_num_rows($result);
         if($count == 1){
            ...
            $sql = "UPDATE users SET recoveryCode = '{$recoveryCode}' WHERE user_email = '{$email}' AND username = '{$username}' ";
            $result = query($sql);
            if(!$result){
              die("Query failed due to ".mysqli_error($connection));
            }
            ...
            $msg = "
                    ...
                    http://yestocode.com/healthservices/code.php?email=$email&code=$recoveryCode";

            $headers = "From: noreply@loginApp.com";
