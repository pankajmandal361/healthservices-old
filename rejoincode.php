...
<?php
  if(isset($_COOKIE['temp_access_code'])){
    if(isset($_GET['email']) && isset($_GET['code'])){
      ...
      $sql = "SELECT recoveryCode FROM doctors WHERE doc_email = '{$email}' ";
      $result = query($sql);
      if(!$result){
      ...
      if(row_count($result) == 1){
        ...
        if($recoveryCode === $code){
          $sql = "UPDATE doctors SET recoveryCode = 'ZEB-nc4300' WHERE doc_email = '{$email}' ";
          $result = query($sql);
          if(!$result){
            die("Query failed due to ".mysqli_error($connection));
          }
          ...
          redirect("docreset.php?email=$email");
        }
        else{
          $message = <<<DELIMITER
